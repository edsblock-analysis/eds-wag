...
function genMarkdown() {
  ...
  // Optional My Account / authenticated-flow analysis — inlined into the consolidated
  // report (not a separate report) when data/myaccount.md is present.
  const maPath = path.join(dataDir, 'myaccount.md');
  if (fs.existsSync(maPath)) {
    let ma = fs.readFileSync(maPath, 'utf8').replace(/^#\s+.*\n/, '');   // drop its top H1
...
function genDetailed() {
  ...
  catalog.forEach(b => {
    ...
    ${b.source ? `<div class="card" style="border-color:var(--me);margin:8px 0"><b style="color:var(--me)">Provenance:</b> ${esc(b.source)} — authenticated/login-gated block identified from the site's JS code; not crawl-verified. Detailed manual analysis required (see §12 of the Full Report).</div>` : ''}
...
function genDashboard() {
  ...
  const data = {
    ...
    hasAuthNote: fs.existsSync(path.join(dataDir, 'myaccount.md')),
...
\${D.hasAuthNote?'<div class="card" style="border-color:var(--me);background:rgba(154,106,0,.06)"><b style="color:var(--me)">⚠︎ Scope note — authenticated flows are high-level only</b><div class="muted" style="margin-top:6px">This analysis covers the <b>public site</b> in full. <b>My Account, checkout and other logged-in / authenticated journeys were assessed at a high level only</b> — automated crawling of them is blocked by <b>Akamai bot protection + login-gating</b>. A <b>detailed analysis of My Account, checkout and the authenticated flow still needs to be performed manually</b> in a logged-in session. High-level findings are in the consolidated report (§12 "My Account & Authenticated / Checkout Flows"): see <a href="reports/full-report.html#">Full Report</a>.</div></div>':''}
...
const nAuth=D.catalog.filter(b=>b.source).length;
document.getElementById('v-blocks').innerHTML='<h2>'+D.catalog.length+' blocks · '+D.totals.variations+' variations</h2>'+(nAuth?'<div class="card" style="border-color:var(--me)"><b style="color:var(--me)">Includes '+nAuth+' authenticated My Account block(s)</b> <span class="muted">identified from the site\\'s JS code (login-gated, not crawl-verified — tagged "auth" below). Detailed manual analysis of these is still required; see §12 of the Full Report.</span></div>':'')+((D.defaultContent&&D.defaultContent.length)?'<div class="card muted">Shared-DOM components are consolidated into one block + variations. <b>EDS default content</b> (not counted as blocks — core decoration / autoblocking handles them): '+D.defaultContent.map(d=>esc(d.name)+' ('+d.pages+')').join(', ')+'.</div>':'')+D.catalog.map(b=>\`<details><summary><span>\${esc(b.name)} <span class="tag">\${esc(b.edsBlock)}</span>\${b.source?' <span class="pill" style="background:rgba(154,106,0,.15);color:var(--me)">auth · code-inferred</span>':''}</span><span><span class="pill \${b.complexity}">\${b.complexity}</span> <span class="muted">\${b.pages} pages</span></span></summary><p style="margin:10px 0 4px"><a href="reports/block-\${esc(b.id)}.html">📄 Detailed report →</a></p><p class="muted">\${esc(b.complexityReason)}</p><h3>Variations (\${b.variations.length})</h3>\${b.variations.map(v=>'<div class="varbox" style="background:var(--panel2);border:1px solid var(--line);border-radius:8px;padding:8px 12px;margin:6px 0"><b>'+esc(v.name)+'</b> <span class="muted">('+v.pages+' pages)</span></div>').join('')}<h3>Functionality</h3><ul class="f">\${b.functional.map(f=>'<li>'+esc(f)+'</li>').join('')}</ul></details>\`).join('');
...
function genEstimates() {
  ...
  <div class="card" style="border-color:var(--me);background:rgba(154,106,0,.06)"><b style="color:var(--me)">⚠︎ Scope — ${esc(host)} only, no subdomains or third parties</b><div class="muted" style="margin-top:6px">This estimate covers the <b>${esc(host)}</b> apex/www site only. It <b>excludes all subdomains and third-party destinations</b>: the <b>login / account flow runs on a separate subdomain</b> (e.g. authenticated portals), and many links either <b>redirect to a subdomain</b> (quote/sales, agent login, commercial/property portals, careers) or to an <b>external third-party domain</b>. Those subdomains and third-party systems are <b>not analyzed and not part of this estimate</b> — each would need its own separate scoping and estimate.</div></div>`;
